...
import (
	...
	"github.com/krol3/starboard/pkg/policyreport"
...
const (
	policyReportCmdShort = "Run static policyReport scanner for each container image of a given workload"
	policyReportCmdLong  = `Scan a given workload for vulnerabilities using Trivy scanner
...
func NewScanPolicyReportsCmd(buildInfo starboard.BuildInfo, cf *genericclioptions.ConfigFlags) *cobra.Command {
	cmd := &cobra.Command{
		Aliases: []string{"policy", "po"},
		Use:     "policyreports (NAME | TYPE/NAME)",
		Short:   policyReportCmdShort,
		Long:    policyReportCmdLong,
...
func ScanPolicyReports(buildInfo starboard.BuildInfo, cf *genericclioptions.ConfigFlags) func(cmd *cobra.Command, args []string) error {
